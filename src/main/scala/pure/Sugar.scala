...
package sugar:
  class Binder(val quantifier: Quantifier, val typ: Type) extends ((List[Var], Expr) => Expr):
    def unapply(expr: Bind): Option[(List[Var], Expr)] =
      expr match {
        case Bind(quantifier, formals, body, `typ`) => Some((formals, body))
        case _ => None
      }
    ...
    def apply(formals: List[Var], body: Expr): Expr = {
      ...
      else Bind(quantifier, formals_, body, typ)
    }
